...
-- super
CREATE USER IF NOT EXISTS '$MYSQL_ROOT_USER'@'172.%.%.%' IDENTIFIED BY '$(cat /run/secrets/db_root_password)';
GRANT ALL PRIVILEGES ON *.* TO '$MYSQL_ROOT_USER'@'172.%.%.%' WITH GRANT OPTION;

-- normal
CREATE USER IF NOT EXISTS '$MYSQL_USER'@'172.%.%.%' IDENTIFIED BY '$(cat /run/secrets/db_password)';
